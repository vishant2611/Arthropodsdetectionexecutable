...
def process_image(image_file, confidence_threshold):
  ...
  storage.child("result_with_nms.jpg"+image_file.name+confidence_threshold).put(result_image_bytes.tobytes())

  # Get the URL of the result image
  result_image_url = storage.child("result_with_nms.jpg"+image_file.name+confidence_threshold).get_url(None)
